...
<body>
	<c:choose>
		<c:when test="${empty sessionScope.user.seq}">
			<script>location.href="planboard/needLogin.jsp";</script>
		</c:when>
		<c:otherwise>
							...
							<td class="budget">
								...
								<div class="input-group mb-1">
									<input type="text" class="form-control" id="ex1"
										placeholder="예) 입장료" maxlength="110"> <input type="text"
										class="form-control" id="money1" placeholder="예) 10000">
...
<script>
$("#schedule-boarder").on('input','input[id^=money]',function() {
	var str = $(this).val();
...
timecheck = true;
$("#schedule-boarder").on('blur','#end-time',function() {
	starttime = $("#start-time").val().split(":");
	endtime = $("#end-time").val().split(":");
	
	if(starttime[0] > endtime[0]) {
		alert("역시작시간이 종료시간보다 늦을 수 없습니다.");
		$("#start-time").val("");
		timecheck = false;
...
$("#success-primary").click(function() {
	...
	for(var i = 0; i < budgetn; i++) {
		if($("#schedule-boarder>tbody>tr>.budget div:nth-of-type("+(i+1)+") input[id^='ex']").val() == "" || ($("#schedule-boarder>tbody>tr>.budget div:nth-of-type("+(i+1)+") input[id^='ex']").val() != "" && $("#schedule-boarder>tbody>tr>.budget div:nth-of-type("+(i+1)+") input[id^='money']").val()!="")) {
	...
	if (budgetcheck) { 
	if ((starttime == "" || endtime == "") && timecheck) {
		alert("시간을정해주세요");
	} else if (place == "") {
		alert("장소를정해주세요");
	} else if (schedule == "") {
